...
@Component({
  selector: 'ngx-update-prospect',
  templateUrl: './update-prospect.component.html',
  styleUrls: ['./update-prospect.component.scss']
})
export class UpdateProspectComponent implements OnInit {
  ...
  modifForm = this.fb.group({
    ...
    prospc_ninea: [''],           // obligatoire si personne morale
    prospc_typesociete: [''],     // obligatoire si personne morale
    prospc_registrecommerce: [''], // obligatoire si personne morale
  ...
  ngOnInit(): void {
    ...
    //this.prospc_titreCivilite = this.prospect.prospc_titre.toString();
    ...
    this.modifForm.controls['prospc_princdirigeant'].setValue(this.prospect.prospc_princdirigeant);
    ...
    this.modifForm.controls['prospc_utilisateur'].setValue(this.prospect.prospc_utilisateur);
    ...
    if (this.prospect.prospc_classificationmetier != null) {
      this.classifCtrl.setValue(this.prospect.prospc_classificationmetier);
    }
    ...
    this.modifForm.controls['prospc_passeport'].setValue(this.prospect.prospc_passeport);
    ...
    if (this.typePersonneChoisi === this.personneMorale) {
      ...
      this.modifForm.controls['prospc_categosocioprof'].clearValidators();
      ...
      if(this.prospect.prospc_activitecommercante == this.NON){
        ...
        this.modifForm.controls['prospc_email'].clearValidators();
      } else {
        this.showObligatoireCINetPasseport = false ;
  ...
  // onGetValueofSecteurActivite(numero: number){
  //   return  (this.classifications?.find(c => c.code == numero))?.libelle;
  // }
  ...
  onGetLibelleByCategorieSociopro(numero: any) {
    // console.log(this.codeCategorieSociopro) ;
  ...
  onChangeCivilite(event) {
    ...
    /*
    this.modifForm.controls['prospc_titre'].setValue(event);
    this.typePersonneChoisi = this.modifForm.get("prospc_nature").value;
    this.civiliteNature = this.modifForm.get("prospc_titre").value;
    
    if (this.typePersonneChoisi == this.personnePhysique && this.civiliteNature == this.gerant.toString()) {
      this.code_typeSociete = 8;
      // this.problemePersonnePhysiCivilite = true;
      this.idTypeSociete = 8;
      this.listTypeSociete = this.listTypeProspect['TYPE_SOCIETE'];
      this.civile = (this.listTypeSociete.find(p => p.id == this.idTypeSociete))?.value;
      this.modifForm.controls['prospc_typesociete'].setValue(this.civile);
      //this.code_typeSociete = this.idTypeSociete.toString() ;
    }
    else {
      // this.problemePersonnePhysiCivilite = false;
    }
  ...
  onChangeClassification(event) {
    // this.modifForm.controls['prospc_classificationmetier'].setValue(event);
  ...
  onSubmit() {
    ...
    */
    ...
    if (this.typePersonneChoisi === this.personnePhysique && this.nom === null) {
      ...
      this.erreur = true;
    } else if (this.typePersonneChoisi === this.personnePhysique && this.categoriesociopro === null) {
      this.problemeCategorieSociopro = true;
      this.erreur = true;
    } 
    // else if (this.typePersonneChoisi === this.personnePhysique && (this.cin === null || this.cin === "")) {
    //   this.problemeCIN = true;
    //   this.erreur = true;
    // } 
    // else if (this.typePersonneChoisi === this.personnePhysique && this.cin.length != this.longeurCIN) {
    ...
    // else if (this.typePersonneChoisi === this.personneMorale && this.activiteCommercante === this.OUI && ((this.codeNinea === null || this.codeNinea === "") || (this.codeRC === null || this.codeRC === ""))) {
    //   this.problemeCodeNineaEtRC = true
    //   this.erreur = true;
    // }
    ...
    //   && this.listeProspects.find(p => p.prospc_ninea?.toLowerCase() === this.codeNinea?.toLowerCase()
    //     || p.prospc_registrecommerce?.toLowerCase() === this.codeRC?.toLowerCase())) {


    //   if (this.listeProspects.find(p => p.prospc_ninea?.toLowerCase() === this.codeNinea?.toLowerCase()
    //     || p.prospc_registrecommerce?.toLowerCase() === this.codeRC?.toLowerCase()).prospc_numero != this.numeroProspect) {
    ...
    // On vérifie qu'il n'y a pas de personne morale qui existe déjà 
    //  else if (this.denomination != null && this.codeNinea != null && this.codeRC != null
    //   && this.listeProspects.find(p => p.prospc_denomination?.toLowerCase() === this.denomination?.toLowerCase()
    //     && p.prospc_ninea?.toLowerCase() === this.codeNinea?.toLowerCase() && p.prospc_registrecommerce?.toLowerCase() === this.codeRC?.toLowerCase() && p.prospc_numero != this.numeroProspect)) {
    ...
    */
      ...
      this.prospectService.updateProspect(this.modifForm.value)
        .subscribe((data) => {
          ...
          (error) => {
            this.toastrService.show(
              //"Echec de la modification du prospect.",
              error.error.message,
  ...
  onFocusOutEventTypeSociete() {
    ...
    this.typeSociete = this.modifForm.get("prospc_typesociete").value;
    /*
    if (this.typePersonneChoisi === this.personneMorale && this.typeSociete === null) {
      this.problemeTypeSociete = true;
      this.problemePersonneMorale = false;
      this.erreur = true
    } else {
