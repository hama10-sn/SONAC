...
@Component({
  selector: 'ngx-perte-acte',
  templateUrl: './perte-acte.component.html',
  styleUrls: ['./perte-acte.component.scss']
})
export class PerteActeComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,private transfertData: TransfertDataService,private demSocService:DemandephysiqueService,
  ...
  ngOnInit(): void {
    ...
    if(data!=null){
      ...
      this.addForm.controls['acte_montant_mensuel'].setValue(Math.trunc(this.demandePhysique.dem_montant/12));
  ...
  onExportActePerte(demande:Dem_Pers) {
    ...
    this.demSocService.generateReportActeLocassur(demande.dem_persnum,form)
        .subscribe(event => {
        let message=""
        switch (event.type) {
  ...
  onExportActePertebis(demande:Dem_Pers) {
    //this.police=this.addForm.get("acte_titre").value;
