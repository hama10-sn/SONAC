...
@Component({
  selector: 'ngx-credit-acte',
  templateUrl: './credit-acte.component.html',
  styleUrls: ['./credit-acte.component.scss']
})
export class CreditActeComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,private transfertData: TransfertDataService,private demPersService:DemandephysiqueService,
  ...
  onExportActeCredit(demande:Dem_Pers) {
    ...
    this.addForm.controls['acte_type_prod'].setValue(this.demandePhysique.dem_produitdemande1);
    ...
    this.demPersService.generateReportActeCredit(demande.dem_persnum,form)
        .subscribe(event => {
        let message=""
        switch (event.type) {
          ...
          case HttpEventType.ResponseHeader:
            ...
            if (event.status == 200) {
              ...
              if(!this.haveActe){
                ...
                this.demPersService.update(demande).subscribe(data => {
  ...
  onExportActeCreditbis(demande:Dem_Pers) {
    this.titre=this.addForm.get("acte_titre").value;
   this.dossier=this.addForm.get("acte_dossier").value;
   this.acheteur=this.addForm.get("acte_acheteur").value;
   this.date=dateFormatter(this.addForm.get("acte_date").value,"yyyy-MM-dd");
  ...
  getCientById(id) {
    this.clientService.getClient(id)
      .subscribe((data: any) => {
        ...
        this.clients=this.client.clien_prenom+" "+this.client.clien_nom;
  ...
  getProspectById(id) {
    this.prospectService.getProspectByNumero(id)
      .subscribe((data: any) => {
        ...
        this.clients=this.prospect.prospc_prenom+" "+this.prospect.prospc_nom;
      ...
      .subscribe((data: any) => {
        ...
        if(this.polices!=null){
          for(let i=0;i<this.polices.length;i++){
            ...
            this.getAllEngagementByPolice(this.polices[i].poli_numero,this.polices[i].poli_codeproduit);
          }
    ...
    this.engagementService.getAllEngagementsByPolice(id)
      .subscribe((data: any) => {
        ...
        this.engagements.forEach(engagement =>{
          ...
          if(produitPolice==this.demandePhysique.dem_produitdemande1 && engagement.engag_status=="en cours"){
