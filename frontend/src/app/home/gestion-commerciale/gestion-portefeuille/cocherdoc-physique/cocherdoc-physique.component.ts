...
@Component({
  selector: 'ngx-cocherdoc-physique',
  templateUrl: './cocherdoc-physique.component.html',
  styleUrls: ['./cocherdoc-physique.component.scss']
})
export class CocherdocPhysiqueComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dem_Pers =  this.transfertData.getData()
    console.log('mbengue',this.dem_Pers?.list_document_valide);
    if(this.dem_Pers.dem_statut)
      if(this.dem_Pers.dem_statut==="analyse de risque"){
    ...
    //if(this.dem_Pers.list_document_valide!=null){
      /* if(this.dem_Pers.dem_produitdemande1==15001001){
        ...
        if(this.dem_Pers?.list_document_valide[0]==4){
          this.checked=true
        }else{
          this.checked=false;
        }
        ...
        if(this.dem_Pers?.list_document_valide[1]==11){
          this.checkeddeux=true;
        }else{
          this.checkeddeux=false
        }
        ...
        if(this.dem_Pers?.list_document_valide[4]==14){
      ...
      /* else if(this.dem_Pers.dem_produitdemande1==14001001){
        ...
        if(this.dem_Pers?.list_document_valide[0]==17){
          ...
          if(this.dem_Pers?.list_document_valide[4]==22){
            this.checkedquatre=true;
          }else{
            this.checkedquatre=false
          }
    ...
    } */if(this.dem_Pers.dem_statut==this.type){
      if(this.dem_Pers?.list_document_valide[0]==30){
        this.checked=true
      }else{
        this.checked=false;
      }
      if(this.dem_Pers?.list_document_valide[1]==31){
        this.checkeddeux=true;
      }else{
        this.checkeddeux=false
      }
      if(this.dem_Pers?.list_document_valide[2]==32){
        this.checkedtrois=true;
      }else{
        this.checkedtrois=false
      }
      if(this.dem_Pers?.list_document_valide[3]==33){
        this.checkedquatre=true;
      }else{
        this.checkedquatre=false
      }
  ...
  //console.log(this.dem_Pers);
  //console.log(this.dem_Pers.dem_persnum);
    ...
    this.demPService.getFilesDoc(dem_persnum)
      .subscribe(data => {
        ...
        if(this.filenames==null){
          ...
          if(this.dem_Pers.dem_statut!="analyse de risque" && this.dem_Pers.dem_statut!="Rejeté"){
            this.checked=false;
            this.toggle(false);
          }
  ...
  toggle(checked: boolean) {/* 
    ...
    this.checked = checked;
     if(this.dem_Pers.dem_produitdemande1==15001001){
      ...
      if(this.checked==true && this.checkeddeux==true &&this.checkedtrois && this.checkedquatre && this.filenames.length>=4 && this.arraysIdentical(this.dem_Pers.list_document_lu,this.filenames)){
        this.check=true;
        this.ngModelValue="1";
      }else{
        this.check=false;
