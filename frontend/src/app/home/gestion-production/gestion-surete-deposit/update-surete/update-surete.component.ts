...
@Component({
  selector: 'ngx-update-surete',
  templateUrl: './update-surete.component.html',
  styleUrls: ['./update-surete.component.scss']
})
export class UpdateSureteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.modifForm.controls['surete_numero'].setValue(this.surete.surete_numero);
    this.modifForm.controls['surete_numeroengagement'].setValue(this.surete.surete_numeroengagement);
    this.modifForm.controls['surete_numpoli'].setValue(this.surete.surete_numpoli);
    this.onGetClientByPolice(this.surete?.surete_numpoli);
    this.modifForm.controls['surete_numeroavenant'].setValue(this.surete.surete_numeroavenant);
    this.modifForm.controls['surete_numeroacte'].setValue(this.surete.surete_numeroacte);
    this.modifForm.controls['surete_typesurete'].setValue(this.surete.surete_typesurete);
    ...
    this.modifForm.controls['surete_identificationtitre'].setValue(this.surete.surete_identificationtitre);
    this.modifForm.controls['surete_localisation'].setValue(this.surete.surete_localisation);
    this.modifForm.controls['surete_adressegeolocalisation'].setValue(this.surete.surete_adressegeolocalisation);

    this.retenuDeposit = this.formatNumberService.numberWithCommas2(Number(this.surete.surete_retenudeposit));
    this.modifForm.controls['surete_retenudeposit'].setValue(this.surete.surete_retenudeposit);
    if (this.surete?.surete_datedeposit != null) {
      this.dateDeposit = dateFormatter(this.surete?.surete_datedeposit, 'yyyy-MM-ddTh:mm');
    }
    this.modifForm.controls['surete_datedeposit'].setValue(this.dateDeposit);
    this.modifForm.controls['surete_cautionsolidaire'].setValue(this.surete.surete_cautionsolidaire);
    ...
    this.modifForm.controls['surete_depositlibere'].setValue(this.surete.surete_depositlibere);
    this.modifForm.controls['surete_dateliberation'].setValue(this.surete.surete_dateliberation);
    this.modifForm.controls['surete_datecomptabilisation'].setValue(this.surete.surete_datecomptabilisation);
    this.modifForm.controls['surete_statutliberation'].setValue(this.surete.surete_statutliberation);
  ...
  */
  ...
  onChangeEngagement(event) {
    this.modifForm.controls['surete_numeroengagement'].setValue(event.value);
    // this.onInsererChamps(event.value);
  }

  onChangeFocusDateDeposit(event: any) {
    console.log(this.dateEcheance);
    this.deposite = this.modifForm.controls['surete_datedeposit'].value;
    ...
    if (dateFormatter(this.deposite, 'yyyy-MM-dd') >= dateFormatter(this.effet, 'yyyy-MM-dd') && dateFormatter(this.dateEcheance, 'yyyy-MM-dd') >= dateFormatter(this.deposite, 'yyyy-MM-dd')) {
      ...
      this.modifForm.controls['surete_datedeposit'].setValue("");
    }
  }
  ...
  */
  ...
  onControleByTypeSurete(type: any) {
    ...
    if (type == '1') {
      ...
      this.modifForm.controls['surete_identificationtitre'].setValidators(Validators.required);
      this.modifForm.controls['surete_retenudeposit'].clearValidators();
      this.modifForm.controls['surete_datedeposit'].clearValidators();
      this.modifForm.controls['surete_cautionsolidaire'].clearValidators();
    } else if (type == '2') {
      this.displaytypedepo = true;
      this.displaytype = false;
      this.displayCaution = false;
      this.modifForm.controls['surete_identificationtitre'].clearValidators();
      this.modifForm.controls['surete_retenudeposit'].setValidators(Validators.required);
      this.modifForm.controls['surete_datedeposit'].setValidators(Validators.required);
      this.modifForm.controls['surete_cautionsolidaire'].clearValidators();
    } else if (type == '3') {
      this.displayCaution = true;
      this.displaytype = false;
      this.displaytypedepo = false;
      this.modifForm.controls['surete_cautionsolidaire'].setValidators(Validators.required);
      this.modifForm.controls['surete_retenudeposit'].clearValidators();
      this.modifForm.controls['surete_datedeposit'].clearValidators();
      this.modifForm.controls['surete_identificationtitre'].clearValidators();
    ...
    this.modifForm.controls['surete_identificationtitre'].updateValueAndValidity();
    this.modifForm.controls['surete_retenudeposit'].updateValueAndValidity();
    this.modifForm.controls['surete_datedeposit'].updateValueAndValidity();
    this.modifForm.controls['surete_cautionsolidaire'].updateValueAndValidity();
  }

  onChangeTypeSurete(event) {
    this.modifForm.controls['surete_typesurete'].setValue(event.value);
  ...
  onChangeCaution(event) {
    this.modifForm.controls['surete_cautionsolidaire'].setValue(event);
  }

  onFocusOutEventRetenuDeposit(event) {
    // this.retenuDeposit = this.modifForm.get("surete_retenudeposit").value;
    ...
    this.retenuDeposit = this.formatNumberService.replaceAll(event.target.value, ' ', '');

    if (this.retenuDeposit === null || this.retenuDeposit === '') {
      this.modifForm.controls['surete_retenudeposit'].setValue(0);
      this.retenuDeposit = 0;
      // this.problemeObjectifCA = true;
      // this.erreur = true;
    } else {
      this.retenuDeposit = Number(this.formatNumberService.replaceAll(this.retenuDeposit, ' ', ''));
      this.modifForm.get("surete_retenudeposit").setValue(this.retenuDeposit);
      this.retenuDeposit = this.formatNumberService.numberWithCommas2(this.retenuDeposit);
  ...
  submit() {
    ...
    this.modifForm.controls['surete_codeutilisateur'].setValue(this.user.util_numero);
    if (this.modifForm.controls['surete_typesurete'].value == 2 && this.modifForm.get('surete_retenudeposit').value !== '' && this.modifForm.get('surete_retenudeposit').value != 0 && this.modifForm.controls['surete_statutliberation'].value == 2) {
      this.modifForm.controls['surete_statutliberation'].setValue(1);
    }
    this.sureteService.modifSurete(this.modifForm.value)
      .subscribe((data: any) => {
        if (data.code === "ok") {
