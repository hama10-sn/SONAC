...
@Component({
  selector: 'ngx-ajout-encaissement-avoir',
  templateUrl: './ajout-encaissement-avoir.component.html',
  styleUrls: ['./ajout-encaissement-avoir.component.scss']
})
export class AjoutEncaissementAvoirComponent implements OnInit {
  ...
  montantAvoir: any = '';
  afficheMontantAvoir: boolean = false;
  afficheMessageNumeroTitre: boolean = false;
  erreurPaiement: boolean = false;
  afficheMessageMontantAvoirZero: boolean = false;
  ...
  checkMontantPay(montant) {
    ...
    if ((Number(montant) > Number(this.addForm.controls['encai_mtnquittance'].value)) || (Number(montant) > this.montantAvoir)) {
      this.showErrorMontantPay = true;
      this.erreurPaiement = true;
    } else {
      this.showErrorMontantPay = false;
      this.erreurPaiement = false;
    }
  }

  checkMontantPayMultiple(montant) {

    if ((Number(montant) > Number(this.SomFactures)) || (Number(montant) > this.montantAvoir)) {
      this.showErrorMontantPay = true;
      this.erreurPaiement = true;
    } else {
      this.showErrorMontantPay = false;
  ...
  submit() {
    ...
    this.encaissementService.saveEncaissementAvoir(this.addForm.value)
      .subscribe((data: any) => {
        console.log(data)
        ...
        // Maintenant on peut mettre à jour les références de l'encaissement
        this.refEncaissementService.updateRefEncaissement(this.addFormRefEncaissement.value)
          .subscribe((data: any) => {
            console.log(data)
            if (data.code === 'ok') {
  ...
  onChangeMontPay2(event) {
    ...
    if (event.target.value == 0 || event.target.value == '') {
      ...
      this.MontantCheque = event.target.value;
      // this.checkMontantPay(event.target.value);
      this.checkMontantPayMultiple(event.target.value);
    }
    ...
    if (this.MontantCheque != 0 && this.SomFactures != 0) {
      ...
      this.encaissementService.addEncaissementMultiple(ligneFactures.value, this.MontantCheque)
        .subscribe((data: any) => {
          ...
          // Maintenant ajouter les références de l'encaissement
          this.refEncaissementService.updateRefEncaissement(this.addFormRefEncaissement.value)
            .subscribe((data: any) => {
              console.log(data)
              if (data.code === 'ok') {
  ...
  onChangeNumeroTitre(event) {
    console.log(event.target.value);

    this.addForm.get('encai_mtnpaye').setValue('');
    this.addFormMultipleEncaiss.get('encai_mtnpaye').setValue('');

    this.refEncaissementService.getRefEncaissementByNumeroTitre(event.target.value)
      .subscribe((data: any) => {
        console.log(data);

        if (data.code === 'ok') {
          this.montantAvoir = data.data.refencai_montantavoir;
          this.afficheMontantAvoir = true;
          this.afficheMessageNumeroTitre = false;
          if (this.montantAvoir <= 0) {
            this.erreurPaiement = true;
            this.afficheMessageMontantAvoirZero = true;
          } else {
            this.erreurPaiement = false;
            this.afficheMessageMontantAvoirZero = false;
          }

        } else {
          this.montantAvoir = '';
          this.afficheMontantAvoir = false;
          this.afficheMessageNumeroTitre = true;
          this.erreurPaiement = true;
        }
      });
  }
