...
@Component({
  selector: 'ngx-encaissement-annuler',
  templateUrl: './encaissement-annuler.component.html',
  styleUrls: ['./encaissement-annuler.component.scss']
})
export class EncaissementAnnulerComponent implements OnInit {
  ...
  public displayedColumns = ['encai_numeroencaissement', 'encai_numerofacture', 'encai_numerosouscripteur', 'encai_datepaiement', 'encai_dateannulation', 'encai_mtnpaye', 'encai_mtnquittance',/*'encai_solde',*/ 'action'];
  ...
  onDownloadRecuFactureSimple(encaissement: Encaissement) {
    ...
    this.encaissementService.downloadRecuEncaissementAnnuler(encaissement.encai_numeroencaissement, form)
      .subscribe(event => {
        switch (event.type) {
          ...
          case HttpEventType.Response:
            saveAs(event.body, 'reçu facture annuler.pdf');
        }
      });
  }
  ...
  getMontantPrimeEncaisser(numFactEncaissement) {
