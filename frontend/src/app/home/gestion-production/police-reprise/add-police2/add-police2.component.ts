...
import { AjoutLot2Component } from './ajout-lot2/ajout-lot2.component';
import { AjoutAcheteur2Component } from './ajout-acheteur2/ajout-acheteur2.component';

@Component({
  selector: 'ngx-add-police2',
  templateUrl: './add-police2.component.html',
  styleUrls: ['./add-police2.component.scss']
})
export class AddPolice2Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.myForm.get('policeForm.poli_dateeffetencours').setValue(dateFormatter('2022-01-01 00:00:00', 'yyyy-MM-ddThh:mm'));
    this.myForm.get('policeForm.poli_dateecheance').setValue(dateFormatter('2022-12-31 00:00:00', 'yyyy-MM-ddThh:mm'));
  ...
  onChangeAcc(event) {
    ...
    this.accessoir = event.target.value;
  ...
  ajoutLot(dialog: TemplateRef<any>) {
    const lots = this.myForm.get('lots') as FormArray;
    if (this.nbLots > lots.length) {
      this.dialogService.open(AjoutLot2Component).onClose
  ...
  ajoutAcheteur(dialog: TemplateRef<any>) {
    ...
    const acheteurs = this.myForm.get('acheteurs') as FormArray;
    if (this.nbAcheteur > acheteurs.length) {
      this.dialogService.open(AjoutAcheteur2Component, {
