...
@Component({
  selector: 'ngx-encaissement-moratoire-penalite',
  templateUrl: './encaissement-moratoire-penalite.component.html',
  styleUrls: ['./encaissement-moratoire-penalite.component.scss']
})
export class EncaissementMoratoirePenaliteComponent implements OnInit {
  title = 'La liste des recours';
  autorisation = [];
  ...
  public displayedColumns = ['sin_client', 'sin_police', 'sin_num', 'mvts_num', 'mvts_typemvt', 'action'];
  ...
  onGetAllSinistresRecours() {
    this.recoursService.listeMoratoires()
      .subscribe((data: any) => {
  ...
  onOpen(dialog: TemplateRef<any>, mvts: Mvtsinistre) {
    ...
    this.sinistreService.getMoratoireBySinistre(mvts.mvts_numsinistre)
      .subscribe((data: any) => {
        this.moratoire = data.data;
      })
  }

  onEncaisserMoratoire(mvts: Mvtsinistre) {
    this.sinistreService.getSinistreByNumero(mvts.mvts_numsinistre)
      .subscribe((data: any) => {
        let sinistre = data.data;
        this.transfertData.setData(sinistre);
        this.router.navigateByUrl('/home/gestion-moratoire/encaisser-moratoire');
      })
  }

  onEncaisserPenalite(mvts: Mvtsinistre) {
    this.sinistreService.getSinistreByNumero(mvts.mvts_numsinistre)
      .subscribe((data: any) => {
        let sinistre = data.data;
        this.transfertData.setData(sinistre);
        this.router.navigateByUrl('/home/gestion-moratoire/encaisser-penalite');
      })
  }
  ...
  onDownloadRecuPenalite(mvts: Mvtsinistre) {
    let demandeurNew = this.demandeur.replace(/ /g, "_");
    const form = new FormData();
    form.append("demandeur", demandeurNew);

    this.recoursService.downloadRecuEncaissementPenalite(mvts.mvts_numsinistre, form)
      .subscribe(event => {
        switch (event.type) {
          ...
          case HttpEventType.Response:
            saveAs(event.body, 'reçu encaissement pénalité.pdf');
        }
      });
  }

  onDownloadRecuMoratoire(mvts: Mvtsinistre) {
    let demandeurNew = this.demandeur.replace(/ /g, "_");
    const form = new FormData();
    form.append("demandeur", demandeurNew);

    this.recoursService.downloadRecuEncaissementMoratoire(mvts.mvts_numsinistre, form)
      .subscribe(event => {
        switch (event.type) {
          case HttpEventType.Sent:
            console.log('Request has been made!');
          ...
          case HttpEventType.UploadProgress:
            break;
          case HttpEventType.Response:
            saveAs(event.body, 'reçu encaissement moratoire.pdf');
        }
      });
  }
