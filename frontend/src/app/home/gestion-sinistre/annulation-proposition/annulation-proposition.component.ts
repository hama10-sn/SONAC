...
@Component({
  selector: 'ngx-annulation-proposition',
  templateUrl: './annulation-proposition.component.html',
  styleUrls: ['./annulation-proposition.component.scss']
})
export class AnnulationPropositionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.myRecoursForm.get('mvtsinistreForm.mvts_typemvt').setValue(11);
  ...
  onSubmit() {
    ...
    this.recoursService.annulerPropositionRecours(this.sinistre.sini_num, this.myRecoursForm.value)
      .subscribe((data: any) => {
        if (data.code === "ok") {
