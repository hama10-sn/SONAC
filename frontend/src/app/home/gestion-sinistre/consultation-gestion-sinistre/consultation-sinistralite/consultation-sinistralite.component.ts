...
import { SinistreFacture } from '../../../../model/SinistreFacture';

@Component({
  selector: 'ngx-consultation-sinistralite',
  templateUrl: './consultation-sinistralite.component.html',
  styleUrls: ['./consultation-sinistralite.component.scss']
})
export class ConsultationSinistraliteComponent implements OnInit {
  title = 'La liste des sinistralités (par ordre de numéro)';
  check: boolean = false ;
  numBranche: number = 0;
  numProduit: number = 0;
  numClient: number = 0;
  ...
  public displayedColumns = ['sin_client', 'sin_police', 'sin_branche', 'fact_montantprimenet', 'fact_montantaccescompagnie', 'sini_sapglobale', 'sini_reglementglobal', 'sini_recoursglobal', 'sinistralite', 'action'];
  ...
  onGetAllSinistralite() {
    this.sinistreService.getAllSinistralite()
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }

  onGetAllSinistraliteByBranche(numBranche: number) {
    this.sinistreService.getSinistraliteByBranche(numBranche)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }

  onGetAllSinistraliteByPeriode(debut: string, fin: string) {
    this.sinistreService.getSinistraliteByPeriode(debut, fin)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }

  onGetAllSinistraliteByClient(numClient: number) {
    this.sinistreService.getSinistraliteByClient(numClient)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }

  onGetAllSinistraliteByPolice(numPolice: number) {
    this.sinistreService.getSinistraliteByPolice(numPolice)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }

  onGetAllSinistraliteByBrancheAndPeriode(numBranche: number, debut: string, fin: string) {
    this.sinistreService.getSinistraliteByBrancheAndPeriode(numBranche, debut, fin)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }

  onGetAllSinistraliteByBrancheAndPeriodeAndClient(numBranche: number, debut: string, fin: string, numClient: number) {
    this.sinistreService.getSinistraliteByBrancheAndPeriodeAndClient(numBranche, debut, fin, numClient)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }

  onGetAllSinistraliteByBrancheAndPeriodeAndClientAndPolice(numBranche: number, debut: string, fin: string, numClient: number, numPolice: number) {
    this.sinistreService.getSinistraliteByBrancheAndPeriodeAndClientAndPolice(numBranche, debut, fin, numClient, numPolice)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      })
  }
  ...
  onConsulter() {
    if (this.numBranche != 0 && (this.debut == '0') && (this.fin == '0') && this.numClient == 0 && this.numPolice == 0) {
      this.onGetAllSinistraliteByBranche(this.numBranche);
      this.title = 'La liste des sinistralités (par branche)' ;
    }  else if(this.numBranche == 0 && (this.debut != '0' && this.debut !== '') && (this.fin != '0' && this.fin !== '') && this.numClient == 0 && this.numPolice == 0) {
      this.onGetAllSinistraliteByPeriode(this.debut, this.fin);
      this.title = 'La liste des sinistralités (par période)' ;
    } else if(this.numBranche == 0 && (this.debut == '0') && (this.fin == '0') && this.numClient != 0 && this.numPolice == 0) {
      this.onGetAllSinistraliteByClient(this.numClient);
      this.title = 'La liste des sinistralités (par client)' ;
    } else if(this.numBranche == 0 && (this.debut == '0') && (this.fin == '0') && this.numClient == 0 && this.numPolice != 0) {
      this.onGetAllSinistraliteByPolice(this.numPolice);
      this.title = 'La liste des sinistralités (par police)' ;
    } else if(this.numBranche != 0 && (this.debut != '0') && (this.fin != '0') && this.numClient == undefined && this.numPolice == undefined) {
      this.onGetAllSinistraliteByBrancheAndPeriode(this.numBranche, this.debut, this.fin);
      this.title = 'La liste des sinistralités (par branche et période)' ;
    } else if(this.numBranche != 0 && (this.debut != '0') && (this.fin != '0') && this.numClient != 0 && this.numPolice == undefined) {
      this.onGetAllSinistraliteByBrancheAndPeriodeAndClient(this.numBranche, this.debut, this.fin, this.numClient);
      this.title = 'La liste des sinistralités (par branche, période et client)' ;
    } else if(this.numBranche != 0 && (this.debut != '0') && (this.fin != '0') && this.numClient != 0 && this.numPolice != 0) {
      this.onGetAllSinistraliteByBrancheAndPeriodeAndClientAndPolice(this.numBranche, this.debut, this.fin, this.numClient, this.numPolice);
      this.title = 'La liste des sinistralités (par branche, période, client et police)' ;
    } else {
      this.onGetAllSinistralite();
      this.title = 'La liste des sinistralités (par ordre de numéro)' ;
    }
    this.check = true ;
  }

  onOpen(dialog: TemplateRef<any>, sinistre: SinistreFacture) {
    this.sinistreService.getDetailsSinistraliteByBranche(sinistre.sini_branche)
      .subscribe((sinistre: any) => {
  ...
  onExport(format: String) {
    ...
    if (format === 'pdf') {
      this.sinistreService.generateReportSinistralite(format, title, demandeur, this.numBranche, this.debut, this.fin, this.numClient, this.numPolice)
        .subscribe(event => {
          switch (event.type) {
            ...
            case HttpEventType.Response:
              saveAs(event.body, 'liste des sinistralités.pdf');
          }
        });
    }
        ...
        .subscribe(event => {
          switch (event.type) {
            ...
            case HttpEventType.UploadProgress:
              break;
            case HttpEventType.Response:
              saveAs(event.body, 'liste des sinistralités.xlsx');
          }
        });
    }
