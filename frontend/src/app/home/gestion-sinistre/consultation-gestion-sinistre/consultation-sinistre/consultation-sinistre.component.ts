...
@Component({
  selector: 'ngx-consultation-sinistre',
  templateUrl: './consultation-sinistre.component.html',
  styleUrls: ['./consultation-sinistre.component.scss']
})
export class ConsultationSinistreComponent implements OnInit, AfterViewInit {
  ...
  onGetAllSinistreByBranche(numBranche: number) {
    this.sinistreService.getSinistreByBranche(numBranche)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }

  onGetAllSinistreByProduit(numProduit: number) {
    this.sinistreService.getSinistreByProduit(numProduit)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }

  onGetAllSinistreByClient(numClient: number) {
    this.sinistreService.getSinistreByClient(numClient)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }

  onGetAllSinistreByPolice(numPolice: number) {
    this.sinistreService.getSinistreByPolice(numPolice)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }
  ...
  onGetAllSinistreByBrancheAndProduit(numBranche: number, numProduit: number) {
    this.sinistreService.getSinistreByBrancheAndProduit(numBranche, numProduit)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }

  onGetAllSinistreByBrancheAndProduitAndPeriode(numBranche: number, numProduit: number, debut: string, fin: string) {
    this.sinistreService.getSinistreByBrancheAndProduitAndPeriode(numBranche, numProduit, debut, fin)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }

  onGetAllSinistreByBrancheAndProduitAndPeriodeAndClient(numBranche: number, numProduit: number, debut: string, fin: string, numClient: number) {
    this.sinistreService.getSinistreByBrancheAndProduitAndPeriodeAndClient(numBranche, numProduit, debut, fin, numClient)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }

  onGetAllSinistreByBrancheAndProduitAndPeriodeAndClientAndPolice(numBranche: number, numProduit: number, debut: string, fin: string, numClient: number, numPolice: number) {
    this.sinistreService.getSinistreByBrancheAndProduitAndPeriodeAndClientAndPolice(numBranche, numProduit, debut, fin, numClient, numPolice)
      .subscribe((data: any) => {
        this.dataSource.data = data;
      });
  }
  ...
  onConsulter() {
    if (this.numBranche != 0 && this.numProduit == 0 && (this.debut == '0') && (this.fin == '0') && this.numClient == 0 && this.numPolice == 0) {
      this.onGetAllSinistreByBranche(this.numBranche);
      this.title = 'La liste des sinistres (par branche)' ;
    } else if(this.numBranche == 0 && this.numProduit != 0 && (this.debut == '0') && (this.fin == '0') && this.numClient == 0 && this.numPolice == 0) {
      this.onGetAllSinistreByProduit(this.numProduit);
      this.title = 'La liste des sinistres (par produit)' ;
    } else if(this.numBranche == 0 && this.numProduit == 0 && (this.debut != '0' && this.debut !== '') && (this.fin != '0' && this.fin !== '') && this.numClient == 0 && this.numPolice == 0) {
      this.onGetAllSinistreByPeriode(this.debut, this.fin);
      this.title = 'La liste des sinistres (par période)' ;
    } else if(this.numBranche == 0 && this.numProduit == 0 && (this.debut == '0') && (this.fin == '0') && this.numClient != 0 && this.numPolice == 0) {
      this.onGetAllSinistreByClient(this.numClient);
      this.title = 'La liste des sinistres (par client)' ;
    } else if(this.numBranche == 0 && this.numProduit == 0 && (this.debut == '0') && (this.fin == '0') && this.numClient == 0 && this.numPolice != 0) {
      this.onGetAllSinistreByPolice(this.numPolice);
      this.title = 'La liste des sinistres (par police)' ;
    } else if(this.numBranche != 0 && this.numProduit != 0 && (this.debut == '0') && (this.fin == '0') && this.numClient == 0 && this.numPolice == 0) {
      this.onGetAllSinistreByBrancheAndProduit(this.numBranche, this.numProduit);
      this.title = 'La liste des sinistres (par branche et produit)' ;
    } else if(this.numBranche != 0 && this.numProduit != 0 && (this.debut != '0') && (this.fin != '0') && this.numClient == undefined && this.numPolice == undefined) {
      this.onGetAllSinistreByBrancheAndProduitAndPeriode(this.numBranche, this.numProduit, this.debut, this.fin);
      this.title = 'La liste des sinistres (par branche, produit et période)' ;
    } else if(this.numBranche != 0 && this.numProduit != 0 && (this.debut != '0') && (this.fin != '0') && this.numClient != 0 && this.numPolice == undefined) {
      this.onGetAllSinistreByBrancheAndProduitAndPeriodeAndClient(this.numBranche, this.numProduit, this.debut, this.fin, this.numClient);
      this.title = 'La liste des sinistres (par branche, produit, période et client)' ;
    } else if(this.numBranche != 0 && this.numProduit != 0 && (this.debut != '0') && (this.fin != '0') && this.numClient != 0 && this.numPolice != 0) {
      this.onGetAllSinistreByBrancheAndProduitAndPeriodeAndClientAndPolice(this.numBranche, this.numProduit, this.debut, this.fin, this.numClient, this.numPolice);
      this.title = 'La liste des sinistres (par branche, produit, période, client et police)' ;
    } else {
      this.onGetAllSinistre();
      this.title = 'La liste des sinistres (par ordre de numéro)' ;
    }
    this.check = true ;
  }
