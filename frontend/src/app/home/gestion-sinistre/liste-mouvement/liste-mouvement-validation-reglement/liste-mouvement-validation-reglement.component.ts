...
@Component({
  selector: 'ngx-liste-mouvement-validation-reglement',
  templateUrl: './liste-mouvement-validation-reglement.component.html',
  styleUrls: ['./liste-mouvement-validation-reglement.component.scss']
})
export class ListeMouvementValidationReglementComponent implements OnInit {
  sinistre: Sinistre;
  autorisation = [];
  ...
  onGetMouvementEvaluation() {
    if (this.sinistre != undefined || this.sinistre != null) {
      this.recoursService.listeMouvementParSinistreAndTypeMouvement(this.sinistre.sini_num, 6)
        .subscribe((data: any) => {
          this.dataSource.data = data.data;
        })
    } else {
      this.toastrService.show(
        "Aucun mouvement de validation réglement n'a été faite sur ce sinistre",
