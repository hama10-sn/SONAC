...
@Component({
  selector: 'ngx-liste-mouvement-evaluation',
  templateUrl: './liste-mouvement-evaluation.component.html',
  styleUrls: ['./liste-mouvement-evaluation.component.scss']
})
export class ListeMouvementEvaluationComponent implements OnInit {
  sinistre: Sinistre;
  autorisation = [];
  ...
  onGetMouvementEvaluation() {
    if(this.sinistre != undefined || this.sinistre != null) {
      this.recoursService.listeMouvementParSinistreAndTypeMouvement(this.sinistre.sini_num, 3)
      .subscribe((data: any) => {
        this.dataSource.data = data.data;
      })
    } else {
      this.toastrService.show(
        "Aucun mouvement d'évaluation n'a été faite sur ce sinistre",
