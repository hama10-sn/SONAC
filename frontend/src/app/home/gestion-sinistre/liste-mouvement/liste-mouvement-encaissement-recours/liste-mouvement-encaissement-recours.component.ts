...
@Component({
  selector: 'ngx-liste-mouvement-encaissement-recours',
  templateUrl: './liste-mouvement-encaissement-recours.component.html',
  styleUrls: ['./liste-mouvement-encaissement-recours.component.scss']
})
export class ListeMouvementEncaissementRecoursComponent implements OnInit {
  sinistre: Sinistre;
  autorisation = [];
  ...
  onGetMouvementEvaluation() {
    if (this.sinistre != undefined || this.sinistre != null) {
      this.recoursService.listeMouvementParSinistreAndTypeMouvement(this.sinistre.sini_num, 13)
        .subscribe((data: any) => {
          this.dataSource.data = data.data;
        })
    } else {
      this.toastrService.show(
        "Aucun mouvement d'encaissement de recours n'a été faite sur ce sinistre",
