...
@Component({
  selector: 'ngx-liste-mouvement-validation-recours',
  templateUrl: './liste-mouvement-validation-recours.component.html',
  styleUrls: ['./liste-mouvement-validation-recours.component.scss']
})
export class ListeMouvementValidationRecoursComponent implements OnInit {
  sinistre: Sinistre;
  autorisation = [];
  ...
  onGetMouvementEvaluation() {
    if (this.sinistre != undefined || this.sinistre != null) {
      this.recoursService.listeMouvementParSinistreAndTypeMouvement(this.sinistre.sini_num, 10)
        .subscribe((data: any) => {
          this.dataSource.data = data.data;
        })
    } else {
      this.toastrService.show(
        "Aucun mouvement de validation recours n'a été faite sur ce sinistre",
