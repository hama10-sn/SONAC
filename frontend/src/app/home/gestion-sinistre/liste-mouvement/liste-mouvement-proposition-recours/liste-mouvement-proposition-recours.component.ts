...
@Component({
  selector: 'ngx-liste-mouvement-proposition-recours',
  templateUrl: './liste-mouvement-proposition-recours.component.html',
  styleUrls: ['./liste-mouvement-proposition-recours.component.scss']
})
export class ListeMouvementPropositionRecoursComponent implements OnInit {
  sinistre: Sinistre;
  autorisation = [];
  ...
  onGetMouvementEvaluation() {
    if (this.sinistre != undefined || this.sinistre != null) {
      this.recoursService.listeMouvementParSinistreAndTypeMouvement(this.sinistre.sini_num, 9)
        .subscribe((data: any) => {
          this.dataSource.data = data.data;
        })
    } else {
      this.toastrService.show(
        "Aucun mouvement de proposition de recours n'a été faite sur ce sinistre",
