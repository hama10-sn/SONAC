...
@Component({
  selector: 'ngx-liste-mouvement-proposition-reglement',
  templateUrl: './liste-mouvement-proposition-reglement.component.html',
  styleUrls: ['./liste-mouvement-proposition-reglement.component.scss']
})
export class ListeMouvementPropositionReglementComponent implements OnInit {
  sinistre: Sinistre;
  autorisation = [];
  ...
  onGetMouvementEvaluation() {
    if (this.sinistre != undefined || this.sinistre != null) {
      this.recoursService.listeMouvementParSinistreAndTypeMouvement(this.sinistre.sini_num, 5)
        .subscribe((data: any) => {
          this.dataSource.data = data.data;
        })
    } else {
      this.toastrService.show(
        "Aucun mouvement de proposition de réglement n'a été faite sur ce sinistre",
