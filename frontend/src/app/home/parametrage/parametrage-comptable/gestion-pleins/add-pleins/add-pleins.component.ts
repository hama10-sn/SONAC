...
@Component({
  selector: 'ngx-add-pleins',
  templateUrl: './add-pleins.component.html',
  styleUrls: ['./add-pleins.component.scss']
})
export class AddPleinsComponent implements OnInit {
  ...
  onChangeFocusOutMontantCapacite(event) {
    ...
    this.addForm.get('pleins_capacite').setValue(this.montantCapacite);
  ...
  onSubmit() {
    ...
    this.addForm.get('pleins_codeutilisateur').setValue(this.user.util_numero);
    console.log(this.addForm.value);

    this.pleinsService.addPleins(this.addForm.value)
      .subscribe((data: any) => {
        if (data.code === "ok") {
